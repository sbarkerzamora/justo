...
import { IconArrowRight, IconBeach } from "@tabler/icons-react"
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const tool = cc ? getToolForCountry("vacaciones", cc) : getToolBySlug("vacaciones")
  if (!tool) return {}
  ...
  return {
    title: `Calculadora de vacaciones${namePart} | Justo`,
...
export default async function VacationsToolPage({ searchParams }: Props) {
  const { country } = await searchParams
  const cc = country && isValidCountry(country) ? country : null
  const tool: JustoTool | undefined = cc
    ? getToolForCountry("vacaciones", cc)
    : getToolBySlug("vacaciones")
      ...
      <section className="mx-auto w-full max-w-2xl px-6 py-20 sm:py-28">
        ...
        <div className="flex items-center gap-3 mb-6">
          <span className="flex size-8 shrink-0 items-center justify-center rounded-lg border border-border bg-background">
            <IconBeach className="size-4 text-foreground" />
          </span>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link
            ...
            href={cc ? `/es/${cc}?tool=vacations` : "/es/ni?tool=vacations"}
          >
            Abrir calculadora
          ...
          <Link
            ...
            href={cc ? `/docs/legal/${cc.toLowerCase()}/vacaciones` : "/docs/legal"}
          >
            Ver corpus legal
          </Link>
        </div>
      ...
      <section className="mx-auto w-full max-w-2xl px-6 pb-24">
        <div className="rounded-xl border border-border/40 px-6 py-5">
          <p className="text-xs leading-relaxed text-muted-foreground">
            {tool.disclaimer} Si el salario fue variable, hubo pagos parciales, conflicto
            o datos incompletos, valida el resultado con revisión legal o contable profesional.
          </p>
        </div>
      </section>
    </main>
