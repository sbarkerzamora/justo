...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const tool = cc ? getToolForCountry("liquidacion-laboral", cc) : getToolBySlug("liquidacion-laboral")
  if (!tool) return {}
  ...
  return {
    title: `Calculadora de liquidación laboral${namePart} | Justo`,
...
export default async function SettlementToolPage({ searchParams }: Props) {
  const { country } = await searchParams
  const cc = country && isValidCountry(country) ? country : null
  const tool: JustoTool | undefined = cc
    ? getToolForCountry("liquidacion-laboral", cc)
    : getToolBySlug("liquidacion-laboral")
      ...
      <section className="mx-auto w-full max-w-2xl px-6 py-20 sm:py-28">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link
            ...
            href={cc ? `/es/${cc}?tool=settlement` : "/es/ni?tool=settlement"}
          >
            Abrir calculadora
      ...
      <section className="mx-auto w-full max-w-2xl px-6 pb-24">
        <div className="rounded-xl border border-border/40 px-6 py-5">
          <p className="text-xs leading-relaxed text-muted-foreground">
            ...
            compleja, valida el resultado con revisión legal o contable profesional.
          </p>
        </div>
      </section>
    </main>
