...
export async function generateMetadata({
  ...
  const tool = cc
    ? getToolForCountry("generador-contratos", cc)
    : getToolBySlug("generador-contratos")
  if (!tool) return {}

  const countryName = cc ? (countryLabels[cc] ?? cc.toUpperCase()) : null
  const namePart = countryName ? ` en ${countryName}` : ""
  const title = `Generador de contratos de trabajo${namePart} | Justo`

  return getToolPageMetadata({ slug: "generador-contratos", tool, country: cc, title })
}

export default async function ContractToolPage({ searchParams }: Props) {
  const { country } = await searchParams
  const cc = country && isValidCountry(country) ? country : null
        ...
        <section className="mx-auto w-full max-w-2xl px-6 py-20 sm:py-28">
          ...
          <div className="mb-6 flex items-center gap-3">
            <span className="flex size-8 shrink-0 items-center justify-center rounded-lg border border-border bg-background">
              <IconFileDescription className="size-4 text-foreground" />
            </span>
          ...
          <h1 className="text-3xl leading-none font-semibold tracking-[-0.03em] text-foreground sm:text-4xl">
            Generador de contratos de trabajo{namePart}
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              ...
              href={cc ? `/es/${cc}?tool=contract` : "/?tool=contract"}
            >
              Abrir generador
